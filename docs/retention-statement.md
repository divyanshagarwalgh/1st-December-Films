...
- "deleted within two working days": the admin page has a "Delete script text" action that blanks the script, the analysis and the extracted fields while keeping the outcome log. Since 6 Sep 2026 the notification goes through Webflow's native form submission, so the script also sits in the site's Forms tab and in the notification email Webflow sends; the person handling a deletion must delete that submission (Site settings, Forms) and the email from every inbox as well (the admin page says so beside the button). Two working days is the promise to keep, so the inboxes that receive the notification must be checked.

The deletion contact is mail@1stdecember.com as shown on the contact page. The statement now lives in the Designer page; change it there (and in `docs/designer-page-build-sheet.md`) if a different address should receive deletion requests.
