...
- "deleted within two working days": the admin page has a "Delete script text" action that blanks the script, the analysis and the extracted fields while keeping the outcome log. The notification email also carries the full script, so the person handling a deletion must delete that email from every inbox in NOTIFY_TO as well (the admin page says so beside the button). Two working days is the promise to keep, so the inbox that receives the notification must be checked.
